      ...
      <div className="bg-white rounded-xl shadow p-5 mb-6">
        ...
        <div className="mt-3">
          {!cameraOn ? (
            ...
            <button onClick={stopCamera} className="bg-gray-800 text-white px-4 py-2 rounded font-medium">
              Tutup Kamera
            </button>
          )}
        </div>
        ...
        {loading && <p className="text-sm text-gray-500 mt-3">Mencari produk...</p>}
        ...
        <div className="bg-white rounded-xl shadow p-5">
          ...
          <table className="w-full text-sm mb-4">
            ...
            <tbody>
              {product.stocks.map((s) => (
                <tr key={s.warehouseId} className="border-t">
                  <td className="p-2">{s.warehouseName}</td>
                  <td className="p-2">{s.binCode || '-'}</td>
                  <td className="p-2 font-medium">{s.quantity} {product.unit}</td>
                </tr>
              ))}
              {product.stocks.length === 0 && (
                <tr><td colSpan={3} className="p-2 text-center text-gray-400">Belum ada stok</td></tr>
              )}
            </tbody>
          </table>
          ...
          <p className="text-sm mb-4">
            ...
            {product.totalStock <= product.minStock && (
              <span className="text-red-600 ml-2">⚠️ Stok menipis (min {product.minStock})</span>
            )}
          </p>
          ...
          <div className="border-t pt-4">
            ...
            <div className="flex gap-2 mb-3">
              <button
                ...
                className={`px-3 py-1.5 rounded text-sm font-medium ${txType === 'IN' ? 'bg-green-600 text-white' : 'bg-white border'}`}
              >
                ⬇️ Barang Masuk
              </button>
              <button
                onClick={() => setTxType('OUT')}
                className={`px-3 py-1.5 rounded text-sm font-medium ${txType === 'OUT' ? 'bg-red-600 text-white' : 'bg-white border'}`}
              >
                ⬆️ Barang Keluar
              </button>
            </div>
            ...
            {txMsg && (
              <p className={`text-sm mt-3 ${txMsg.startsWith('✓') ? 'text-green-600' : 'text-red-600'}`}>{txMsg}</p>
            )}
          </div>
        </div>
      )}
