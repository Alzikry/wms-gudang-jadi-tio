  ...
  return (
    <div className="min-h-screen wms-bg p-6 relative">
      ...
      <header className="glass-panel flex justify-between items-center px-5 py-4 mb-6">
        <div>
          <h1 className="font-display text-2xl text-ink">Purchase Order</h1>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {orders.map((po) => {
              ...
              return (
                <tr key={po.id} className="border-t border-ink/5 text-ink/85">
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <Link to={`/purchase-orders/${po.id}`} className="text-brand font-semibold hover:underline">Detail / Terima</Link>
                  </td>
                </tr>
              );
            })}
            {orders.length === 0 && (
              <tr><td colSpan={5} className="p-6 text-center text-ink-soft/60">Belum ada Purchase Order</td></tr>
            )}
          </tbody>
        </table>
      </div>
